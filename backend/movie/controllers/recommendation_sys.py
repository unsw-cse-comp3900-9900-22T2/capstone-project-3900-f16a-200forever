...
from movie.utils.recommendation_util import initialise_movies, top_twenty, calculate_genre, calculate_director
...
class RecommendUser(Resource):
  ...
  def get(self):
    ...
    if len(reviews) == 0:
      ...
      # get all movies
      all_movies = initialise_movies()
      calculate_genre(all_movies, user_id)
      calculate_director(all_movies, user_id)
      rec_movies = top_twenty(all_movies)
    return {"movies": convert_model_to_dict(rec_movies)}, 200    
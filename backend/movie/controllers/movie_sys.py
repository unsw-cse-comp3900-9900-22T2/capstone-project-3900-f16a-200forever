...
class GenreMovie(Resource):
  ...
  def get(self):
    ...
    genre_movie_result = db.session.query(Genre.Genres, Movie.MovieGenre, Movie.Movies).with_entities(Movie.Movies.id, Movie.Movies.title, Movie.Movies.backdrop, Movie.Movies.total_rating, Movie.Movies.rating_count).filter(Genre.Genres.id == genre_id).filter(Movie.Movies.id == Movie.MovieGenre.movie_id).filter(Genre.Genres.id == Movie.MovieGenre.genre_id).all()
    movies_lst = []
